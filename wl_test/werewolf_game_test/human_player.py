...
async def _act(self):
    ...
    Now, please speak:
    """
    rsp = self._send_msg(msg=input_instruction)
...
def prepare_human_player(sendMsg,player_class: BasePlayer):
    # Dynamically define a human player class that inherits from a certain role class
    HumanPlayer = type("HumanPlayer", (player_class,), {"_act": _act,"_send_msg":sendMsg})
    return HumanPlayer
